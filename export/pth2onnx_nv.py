...
def parse_args():
    ...
    parser.add_argument(
        "--section", help="section can be either extract_img_feat or pts_head_memory"
    )
...
def main():
    ...
    class TrtPtsHeadContainer(torch.nn.Module):
        ...
        def _post_update_memory(
            ...
            rec_reference_points = topk_gather(
                rec_reference_points, topk_indexes
            ).detach()
            ...
            head.memory_embedding = torch.cat(
                [rec_memory, head.memory_embedding], dim=1
            )
            head.memory_timestamp = torch.cat(
                [rec_timestamp, head.memory_timestamp], dim=1
            )
            head.memory_egopose = torch.cat([rec_ego_pose, head.memory_egopose], dim=1)
            head.memory_reference_point = torch.cat(
        ...
        def _pts_head(self, x, pos_embed, cone):
            ...
            reference_points, attn_mask, mask_dict = head.prepare_for_dn(
                B, reference_points, {}
            )
            ...
            all_bbox_preds[..., 0:3] = (
                all_bbox_preds[..., 0:3] * (head.pc_range[3:6] - head.pc_range[0:3])
                + head.pc_range[0:3]
            )
        ...
        def forward(
            ...
            memory_reference_point = transform_reference_points(
                memory_reference_point, data_ego_pose_inv, reverse=False
            )
    ...
    if args.section == "extract_img_feat":
        ...
        arrs = [
            torch.from_numpy(
                np.random.uniform(-0.5, 0.5, size=(1, 6, 3, 256, 704))
            ).float(),
        ]
        input_names = ["img"]
        ...
        arrs = [
            torch.from_numpy(
                np.random.uniform(-0.5, 0.5, size=(1, 6, 256, 16, 44))
            ).float(),
            ...
            torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, 4, 4))).float(),
            torch.from_numpy(
                np.random.uniform(-0.5, 0.5, size=(1, dmem_init, 256))
            ).float(),
            torch.from_numpy(
                np.random.uniform(-0.5, 0.5, size=(1, dmem_init, 3))
            ).float(),
            torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, 512, 1))).float(),
            torch.from_numpy(
                np.random.uniform(-0.5, 0.5, size=(1, dmem_init, 4, 4))
            ).float(),
            torch.from_numpy(
                np.random.uniform(-0.5, 0.5, size=(1, dmem_init, 2))
            ).float(),
        ]
        input_names = [
            "x",
